-- Database Refactoring Phase 8: Testing and Validation (CORRECTED)
-- This script tests all functionality with the new schema
-- FIXED: Removed all references to non-existent 'role' column in profiles table
...
-- Phase 8.4: Test students migration (CORRECTED - no role column)
DO $$ 
DECLARE
...
BEGIN
  ...
  -- Count original profiles (assuming all are students since no role column)
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'profiles') THEN
    SELECT COUNT(*) INTO profile_students FROM profiles;
    RAISE NOTICE 'Original profiles (all assumed to be students): %', profile_students;
  ELSE
    profile_students := 0;
  ...
  IF profile_students > 0 AND migrated_students < profile_students THEN
    RAISE WARNING 'Some profiles may not have been migrated. Original: %, Migrated: %', 
      profile_students, migrated_students;
  ELSIF profile_students > 0 AND migrated_students = profile_students THEN
    RAISE NOTICE 'All profiles successfully migrated to students table';
  END IF;
...
BEGIN
  ...
  IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'branches') THEN
    RAISE EXCEPTION 'Critical table "branches" is missing';
  END IF;
  
  IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'students') THEN
    RAISE EXCEPTION 'Critical table "students" is missing';
  END IF;
  
  -- Check if resources have been migrated (only if resources table exists)
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'resources') THEN
    IF NOT EXISTS (SELECT FROM resources WHERE title IS NOT NULL LIMIT 1) THEN
      RAISE WARNING 'Resources table exists but appears not to have been migrated properly';
    END IF;
  ELSE
    RAISE NOTICE 'Resources table does not exist yet (may be created in Phase 3)';
  END IF;
  
  RAISE NOTICE '=== TESTS COMPLETED ===';
  RAISE NOTICE 'The database refactoring testing has been completed!';
...
SELECT '=== MANUAL VERIFICATION DATA ===' as section;

-- Show all created lookup data
SELECT 'BRANCHES:' as table_name;
SELECT code, name FROM branches ORDER BY code;

SELECT 'YEARS:' as table_name;
SELECT batch_year, display_name FROM years ORDER BY batch_year DESC;

SELECT 'SEMESTERS:' as table_name;
SELECT s.semester_number, y.batch_year, y.display_name as year_name
FROM semesters s 
JOIN years y ON s.year_id = y.id 
ORDER BY y.batch_year DESC, s.semester_number;

-- Show sample student data if any exists
SELECT 'SAMPLE STUDENTS:' as table_name;
SELECT 
  s.roll_number,
  s.name,
  s.email,
