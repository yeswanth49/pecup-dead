...
BEGIN
  ...
  ELSE
    RAISE NOTICE 'Skipping migration from resources; table does not exist';
  END IF;
END $$;
...
BEGIN
  ...
  ELSE
    RAISE NOTICE 'Skipping file migration; resources table does not exist';
  END IF;
END $$;
...
BEGIN
  ...
  ELSE
    RAISE NOTICE 'Skipping metadata migration; resources table does not exist';
  END IF;
END $$;
...
BEGIN
  ...
  IF EXISTS (
    ...
    SELECT COUNT(*) INTO cnt FROM resources;
    RAISE NOTICE 'Original resources: %', cnt;
  ELSE
    RAISE NOTICE 'Original resources: 0 (table not present)';
  END IF;

  SELECT COUNT(*) INTO cnt FROM resources_new;
  RAISE NOTICE 'New resources created: %', cnt;

  SELECT COUNT(*) INTO cnt FROM resource_files;
  RAISE NOTICE 'Resource files created: %', cnt;

  SELECT COUNT(*) INTO cnt FROM resource_metadata;
  RAISE NOTICE 'Resource metadata entries: %', cnt;

  SELECT COUNT(*) INTO cnt
  FROM resources_new r
  LEFT JOIN resource_files rf ON r.id = rf.resource_id
  WHERE rf.id IS NULL;
  RAISE NOTICE 'Resources without files: %', cnt;
