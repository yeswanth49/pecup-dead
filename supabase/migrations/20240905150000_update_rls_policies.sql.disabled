...
DO $$ BEGIN
  ...
  ELSE
    RAISE NOTICE 'Skipping RLS on audit_logs; table not present';
  END IF;
END $$;
...
CREATE POLICY IF NOT EXISTS "admins_read_all_users" ON users
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY IF NOT EXISTS "superadmins_delete_users" ON users
  FOR UPDATE USING (
    EXISTS (
      SELECT 1 FROM users admin_u
      WHERE admin_u.auth_user_id = auth.uid()
...
-- Users can read their own profile
CREATE POLICY IF NOT EXISTS "user_profiles_read_own" ON user_profiles
  FOR SELECT USING (
...
CREATE POLICY IF NOT EXISTS "user_profiles_insert_own" ON user_profiles
  FOR INSERT WITH CHECK (
    user_id = (SELECT id FROM users WHERE auth_user_id = auth.uid() AND deleted_at IS NULL)
  );

-- Admins can read all profiles
CREATE POLICY IF NOT EXISTS "admins_read_all_profiles" ON user_profiles
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM users u
      WHERE u.auth_user_id = auth.uid()
...
CREATE POLICY IF NOT EXISTS "admins_manage_scoped_profiles" ON user_profiles
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM users admin_u
...
CREATE POLICY IF NOT EXISTS "admin_profiles_read_own" ON admin_profiles
  FOR SELECT USING (
    user_id = (SELECT id FROM users WHERE auth_user_id = auth.uid() AND deleted_at IS NULL)
  );

-- Only superadmins can manage admin profiles
CREATE POLICY IF NOT EXISTS "superadmins_manage_admin_profiles" ON admin_profiles
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM users u
      WHERE u.auth_user_id = auth.uid()
...
CREATE POLICY IF NOT EXISTS "resources_read_relevant" ON resources_new
  FOR SELECT USING (
    deleted_at IS NULL
    AND (
      -- Own resources
...
CREATE POLICY IF NOT EXISTS "resources_insert_own" ON resources_new
  FOR INSERT WITH CHECK (
    uploader_id = (SELECT id FROM users WHERE auth_user_id = auth.uid() AND deleted_at IS NULL)
    AND deleted_at IS NULL
  );
...
CREATE POLICY IF NOT EXISTS "resources_update_own" ON resources_new
  FOR UPDATE USING (
    uploader_id = (SELECT id FROM users WHERE auth_user_id = auth.uid() AND deleted_at IS NULL)
    AND deleted_at IS NULL
  );
...
CREATE POLICY IF NOT EXISTS "admins_manage_resources" ON resources_new
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM users u
      WHERE u.auth_user_id = auth.uid()
...
CREATE POLICY IF NOT EXISTS "resource_files_read_accessible" ON resource_files
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY IF NOT EXISTS "resource_files_manage_own" ON resource_files
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM resources_new r
      WHERE r.id = resource_files.resource_id
...
CREATE POLICY IF NOT EXISTS "admins_manage_resource_files" ON resource_files
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM users u
      WHERE u.auth_user_id = auth.uid()
...
CREATE POLICY IF NOT EXISTS "resource_metadata_read_accessible" ON resource_metadata
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM resources_new r
...
CREATE POLICY IF NOT EXISTS "resource_metadata_manage_own" ON resource_metadata
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM resources_new r
      WHERE r.id = resource_metadata.resource_id
...
CREATE POLICY IF NOT EXISTS "admins_manage_resource_metadata" ON resource_metadata
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM users u
      WHERE u.auth_user_id = auth.uid()
...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='audit_logs') THEN
    EXECUTE $$CREATE POLICY IF NOT EXISTS "audit_logs_insert_authenticated" ON audit_logs
      FOR INSERT WITH CHECK (
        actor_id = (SELECT id FROM users WHERE auth_user_id = auth.uid() AND deleted_at IS NULL)
      )$$;

    EXECUTE $$CREATE POLICY IF NOT EXISTS "admins_read_audit_logs" ON audit_logs
      FOR SELECT USING (
        EXISTS (
          SELECT 1 FROM users u
          WHERE u.auth_user_id = auth.uid()
  ...
  ELSE
    RAISE NOTICE 'Skipping audit_logs policies; table not present';
  END IF;
END $$;
