...
BEGIN
  ...
  ELSE
    RAISE NOTICE 'Skipping resources_new uploader_id backfill; legacy resources not present';
  END IF;
END $$;
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='admin_scopes')
     AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='admins') THEN
  ...
  ELSE
    RAISE NOTICE 'Skipping admin_scopes backfill; legacy tables not present';
  END IF;
END $$;
...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='profiles') THEN
    EXECUTE 'DROP POLICY IF EXISTS "Allow all operations for authenticated users" ON profiles';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='students') THEN
    EXECUTE 'DROP POLICY IF EXISTS "Admins can read all students" ON students';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='resources') THEN
    EXECUTE 'DROP POLICY IF EXISTS "Users can read own resources" ON resources';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='audit_logs') THEN
    EXECUTE 'DROP POLICY IF EXISTS "Admins can manage audit logs" ON audit_logs';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='settings') THEN
    EXECUTE 'DROP POLICY IF EXISTS "Admins can manage settings" ON settings';
  END IF;
END $$;
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='audit_logs') THEN
    RAISE NOTICE 'Skipping audit_logs policy creation in local environment';
  END IF;
END $$;
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='audit_logs') THEN
    EXECUTE 'COMMENT ON TABLE audit_logs IS ''Audit trail for all system actions with unified actor reference''';
  END IF;
END $$;
...
BEGIN
  ...
  SELECT COUNT(*) INTO cnt FROM users; RAISE NOTICE 'Users consolidated: %', cnt;
  SELECT COUNT(*) INTO cnt FROM user_profiles WHERE user_id IS NOT NULL; RAISE NOTICE 'User profiles linked: %', cnt;
  SELECT COUNT(*) INTO cnt FROM admin_profiles WHERE user_id IS NOT NULL; RAISE NOTICE 'Admin profiles linked: %', cnt;

  SELECT COUNT(*) INTO cnt FROM resources_new; RAISE NOTICE 'Resources migrated: %', cnt;
  SELECT COUNT(*) INTO cnt FROM resource_files; RAISE NOTICE 'Resource files linked: %', cnt;
  SELECT COUNT(*) INTO cnt FROM resource_metadata; RAISE NOTICE 'Resource metadata entries: %', cnt;

  SELECT COUNT(*) INTO cnt
  FROM user_profiles up
  LEFT JOIN users u ON up.user_id = u.id
  WHERE u.id IS NULL;
  RAISE NOTICE 'Orphaned user profiles: %', cnt;

  SELECT COUNT(*) INTO cnt
  FROM admin_profiles ap
  LEFT JOIN users u ON ap.user_id = u.id
  WHERE u.id IS NULL;
  RAISE NOTICE 'Orphaned admin profiles: %', cnt;

  SELECT COUNT(*) INTO cnt FROM resources_new r WHERE r.uploader_id IS NULL;
  RAISE NOTICE 'Resources without uploader: %', cnt;

  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='audit_logs') THEN
    SELECT COUNT(*) INTO cnt FROM audit_logs al WHERE al.actor_id IS NULL;
    RAISE NOTICE 'Audit logs without actor: %', cnt;
  ELSE
    RAISE NOTICE 'Audit logs without actor: 0 (audit_logs not present)';
  END IF;

  SELECT COUNT(*) INTO cnt FROM app_settings; RAISE NOTICE 'App settings configured: %', cnt;
END $$;
