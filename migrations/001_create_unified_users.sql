...
-- Phase 1.3: Create user_profiles table (extends users for academic data)
...
BEGIN
  IF EXISTS (
    ...
    SELECT
      ...
      y.id as year_id,
      s.id as semester_id,
      NULL as section,  -- To be populated separately
      p.created_at,
      p.updated_at
    FROM profiles p
    ...
    LEFT JOIN semesters s ON s.year_id = y.id AND s.semester_number = 1  -- Default to semester 1
    WHERE p.role = 'student'
    ON CONFLICT (user_id) DO NOTHING;
    RAISE NOTICE 'Migrated profile data to user_profiles';
  ELSE
    RAISE NOTICE 'profiles table does not exist, skipping user_profiles migration';
  END IF;
END $$;
...
-- Phase 1.5: Verification queries
SELECT '=== MIGRATION 001 VERIFICATION ===' as section;
SELECT 'Users created:' as check, COUNT(*) as count FROM users;
SELECT 'User profiles created:' as check, COUNT(*) as count FROM user_profiles;
SELECT 'Admin profiles created:' as check, COUNT(*) as count FROM admin_profiles;

-- Check for data integrity
SELECT 'Users without profiles (expected for admins):' as check, COUNT(*) as count
FROM users u
LEFT JOIN user_profiles up ON u.id = up.user_id
WHERE up.id IS NULL AND u.role = 'student';

SELECT 'Sample user data:' as check;
SELECT u.email, u.name, u.role, up.roll_number, b.code as branch_code
FROM users u
LEFT JOIN user_profiles up ON u.id = up.user_id
