...
SELECT '=== MIGRATION 005 VERIFICATION ===' as section;
SELECT 'Old settings entries:' as check, COUNT(*) as count FROM settings;
SELECT 'New app_settings entries:' as check, COUNT(*) as count FROM app_settings;

-- Show all settings
SELECT 'Current settings:' as check;
SELECT * FROM get_all_settings();

-- Test helper functions
SELECT 'Test get_setting functions:' as check;
SELECT
  'pdf_to_drive' as setting,
  get_setting_bool('pdf_to_drive') as bool_value,
  get_setting_text('storage_bucket') as text_value,
  get_setting_int('max_file_size_mb') as int_value;

-- Verify backwards compatibility view
SELECT 'Backwards compatibility view:' as check;
