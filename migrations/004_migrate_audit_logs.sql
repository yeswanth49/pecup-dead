...
CREATE OR REPLACE FUNCTION resolve_actor_id(actor_email_param text, actor_role_param admin_role)
RETURNS uuid AS $$
DECLARE
...
UPDATE audit_logs
SET actor_id = resolve_actor_id(actor_email, actor_role::admin_role)
...
FROM (
  SELECT
    ...
    resolve_actor_id(al.actor_email, al.actor_role::admin_role) as resolved_id
  FROM audit_logs al
...
SELECT '=== MIGRATION 004 VERIFICATION ===' as section;
SELECT 'Total audit logs:' as check, COUNT(*) as count FROM audit_logs;
SELECT 'Audit logs with new actor_id:' as check, COUNT(*) as count FROM audit_logs WHERE actor_id IS NOT NULL;
SELECT 'Audit logs missing actor_id:' as check, COUNT(*) as count FROM audit_logs WHERE actor_id IS NULL;

-- Check data integrity
SELECT 'Orphaned audit logs (actor not in users):' as check, COUNT(*) as count
FROM audit_logs al
LEFT JOIN users u ON al.actor_id = u.id
WHERE al.actor_id IS NOT NULL AND u.id IS NULL;

-- Sample migrated audit data
SELECT 'Sample audit logs after migration:' as check;
SELECT
  al.action,
  al.entity,
  al.entity_id,
...
-- Audit log distribution by actor type
SELECT
  'Audit logs by actor role:' as check,
  u.role,
  COUNT(*) as count
FROM audit_logs al
JOIN users u ON al.actor_id = u.id
